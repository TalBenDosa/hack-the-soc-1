...
/** Returns all SHA256-like strings found inside a log object */
...
// ── FieldValue with "Check Hash" button ───────────────────────────────────
...
function FieldValue({ fieldKey, value, onCheckHash }) {
  ...
  return (
    <span className="flex items-center gap-2 flex-wrap">
      <span className="text-white font-mono break-all">{str}</span>
      <button
...
export default function InvestigationLogs({ logs, onSelectLog, selectedLogId, scenarioIOCs = [] }) {
  ...
  const [hashLookup, setHashLookup] = useState(null); // { hash, isMalicious, malwareFamily, fileName }

  // Set of hashes from the scenario's IOC list (definitely malicious)
                      ...
                      >
                        ...
                        <TableCell className="text-white text-sm py-3">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {/* Quick-access hash badge in the row itself */}
                                ...
                                <div className="bg-teal-900/20 border border-teal-500/30 rounded-lg p-3">
                                  ...
                                  <div className="space-y-2">
                                    {hashes.map(({ key, value }) => (
                                      <div key={key} className="flex items-center justify-between gap-3 bg-slate-800/50 rounded p-2">
                                        <div>
                                          <span className="text-xs text-slate-400 mr-2">{key}:</span>
                                          <span className="text-xs font-mono text-slate-200 break-all">{value}</span>
                                        </div>
                                        <button
                                          onClick={() => openHashLookup(value, key)}
                                          className="bg-teal-600 hover:bg-teal-500 text-white text-xs px-3 py-1 rounded transition-colors shrink-0"
                                        >
